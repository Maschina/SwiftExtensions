...
public protocol TypedNotification: Sendable {
...
public protocol TypedNotificationCenter {
	...
	func addObserver<N : TypedNotification>(_ forType: N.Type, sender: N.Sender?, queue: OperationQueue?, dispatch: DispatchQueue?, using block: @Sendable @escaping (N) -> Void) -> NSObjectProtocol
}

extension NotificationCenter : TypedNotificationCenter {
	static let typedNotificationUserInfoKey = "_TypedNotification"
    ...
    @discardableResult
	public func addObserver<N>(_ forType: N.Type, sender: N.Sender? = nil, queue: OperationQueue? = nil, dispatch: DispatchQueue? = DispatchQueue.global(qos: .default), using block: @Sendable @escaping (N) -> Void) -> NSObjectProtocol where N : TypedNotification {
